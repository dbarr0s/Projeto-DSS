...
public class Menu{
    ...
    public interface Handler {
        void execute(); //Executa a ação definida pelo handler.
    }

    //Interface para pré-condições que validam um estado antes da execução.
    public interface PreCondition {
        boolean validate();
        //Valida se uma condição específica é atendida.
        //return true se a condição for válida, false caso contrário.
    }

    //Scanner utilizado para receber entradas do usuário a partir do console.
    ...
    //Construtor vazio: Cria um menu vazio, ao qual se podem adicionar opções.
    public Menu() {
        this.titulo = "Menu";
     ...
     */
    public void runOnce() {
        int op; // Declaração da variável para armazenar a opção do usuário.
        
        show(); // Mostra as opções disponíveis para o usuário.
        
        op = readOption(); // Lê a opção escolhida pelo usuário.
        
        // Verifica se a opção escolhida está dentro do intervalo disponível e se a pré-condição da opção é válida.
        if (op > 0 && !this.disponivel.get(op - 1).validate()) {
            System.out.println("Opção indisponível!"); // Exibe mensagem se a opção não estiver disponível.
        } else if (op > 0) {
            // Executa o handler correspondente à opção selecionada pelo usuário.
